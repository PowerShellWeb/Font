<#
.Synopsis
    GitHub Action for Font
.Description
    GitHub Action for Font.  This will:

    * Import Font
    * If `-Run` is provided, run that script
    * Otherwise, unless `-SkipScriptFile` is passed, run all *.Font.ps1 files beneath the workflow directory
...
param(
# A PowerShell Script that uses Font.  
# Any files outputted from the script will be added to the repository.
# If those files have a .Message attached to them, they will be committed with that message.
[string]
$Run,

# If set, will not process any files named *.Font.ps1
[switch]
$SkipScriptFile,
